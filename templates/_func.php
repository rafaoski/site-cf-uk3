	...
	foreach ($options['items'] as $key => $item) {
		...
		if( isset($item->text_1) ) {
			...
			if( strpos($item->text_1, '@') ) {
				...
				$out .= ' ' . $options['icons'][$key] . '&nbsp;' . sanitizer()->email($item->text_1);
				$out .= "</a>";
			} else {
...
function simpleNav($items, $options = array() )
{
  ...
  // Default Options
	...
	$out .= "<a class='uk-text-uppercase' href='" . $options['parent']->url . "'>";
	$out .= ukIcon('arrow-right', ['ratio' => 1.5]) . ' ' . $options['title'];
	$out .=  "</a>";
...
function pagination($results)
{
	...
	return $results->renderPager(array(
		'nextItemLabel' => setting('next') . ' ' . ukIcon('arrow-right'),
		'previousItemLabel' => ukIcon('arrow-left') . ' ' . setting('previous'),
...
function privacyPolicy($privacyPage, $options = array())
{
	...
	<span class='privacy-text'>
		$options[icon] {$privacyPage->meta_title}
	</span>
